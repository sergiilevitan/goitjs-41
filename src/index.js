...
//     return (parseFloat(weight.replace(',','.')) / Math.pow(parseFloat(height.replace(',','.')),2)).toFixed(1);
// })
...
//     items.forEach((value, id) => console.log(`${id + 1} - ${value} `))
...
// Напиши функцию printContactsInfo(names, phones) которая выводит в консоль имя и телефонный номер
// пользователя.В параметры names и phones будут переданы строки имен и телефонных номеров,
...
//     namesArr.forEach((name, id) => console.log(`${name} - ${phonesArr[id]}` ))
// }
